...
var CentralStore = assign({}, EventEmitter.prototype, {
	...
	getReportDetailData: function (reportTitle) {
		if (_reportDetailData) {
			if (_reportDetailData.title != reportTitle) {
				_reportDetailData = undefined;
			}
		}
...
CentralStore.dispatchToken = Dispatcher.register(function (action) {
	switch (action.type){
		...
		case Actions.LOAD_FOLDERS:
			serverApi.loadFolders(function (err, response, body) {
				if (body.success) {
					_scriptDatas.folders = body.data;
					CentralStore.emit(DASHBOARD_DATA_WATCHER);
				}
			});
			break;
		case Actions.CREATE_FOLDER:
			serverApi.createFolder(action.data, function (err, response, body) {
				if (body.success) {
					_scriptDatas.folders.push(body.data);
		...
		case Actions.UPDATE_FOLDER:
			var data = this.data;
			serverApi.updateFolder(data.folder, data.title, function (err, response, body) {
				if (body.success) {
		...
		case Actions.DELETE_FOLDER:
			serverApi.deleteFolder(action.data, function (err, response, body) {
				if (body.success) {
					var folders = _scriptDatas.folders;
					var index = findArrayIndex(folders, body.data.id);
		...
		case Actions.LOAD_SCRIPTS:
			serverApi.loadScripts(action.data, function (err, response, body) {
				if (body.success) {
					_scriptDatas.scripts = body.data;
					CentralStore.emit(DASHBOARD_DATA_WATCHER);
				}
			});
			break;
		...
		case Actions.LOAD_SCRIPT:
			serverApi.loadScript(action.data, function (err, response, body) {
				if (body.success) {
					var data = body.data;
		...
		case Actions.LOAD_CONFIG_SCRIPTS:
			serverApi.loadConfigScripts(function (err, response, body) {
				if (body.success) {
					for (var key in body.data) {
						_scriptDetailDatas[key] = body.data[key];
					}
					CentralStore.emit(DASHBOARD_DATA_WATCHER);	
				}
			});
		...
		case Actions.SAVE_SCRIPT:
			...
			if (!script.id) {
				serverApi.createScript(script, function (err, response, body) {
					if (body.success) {
						_scriptDetailDatas.script = body.data;
						_penndingTransition = {
							name: 'dashboard',
							params: { section_id: 'script' },
							query: { select_folder: body.data.folder, select_script: body.data.id }
						};
						_responseMessage = {
							target: body.data.id,
				...
				serverApi.updateScript(script, function (err, response, body) {
					if (body.success) {
						_scriptDetailDatas.script = body.data;
						_responseMessage = {
							target: script.id,
		...
		case Actions.DELETE_SCRIPT:
			var script = _scriptDetailDatas.script;
			serverApi.deleteScript(script.id, function (err, response, body) {
				if (body.success) {
		...
		case Actions.LOAD_PARAMETERS:
			serverApi.loadParameters(function (err, response, body) {
				if (body.success) {
					_parameterDatas.parameters = body.data;
					CentralStore.emit(DASHBOARD_DATA_WATCHER);
				}
			});
			break;
		case Actions.CREATE_PARAMETER:
			...
			serverApi.createParameter(data.name, data.value, function (err, response, body) {
				if (body.success) {
					_parameterDatas.parameters.push(body.data);
		...
		case Actions.UPDATE_PARAMETER:
			var data = action.data;
			serverApi.updateParameter(data.id, data.value, function (err, response, body) {
				if (body.success) {
		...
		case Actions.DELETE_PARAMETER:
			serverApi.deleteParameter(action.data, function (err, response, body) {
				if (body.success) {
		...
		case Actions.LOAD_PACKAGES:
			serverApi.loadPackages(function (err, response, body) {
				if (body.success) {
					_packageDatas.packages = body.data;
					CentralStore.emit(DASHBOARD_DATA_WATCHER);
				}
			});
			break;
		...
		case Actions.DELETE_PACKAGE:
			serverApi.deletePackage(action.data, function (err, response, body) {
				if (body.success) {
		...
		case Actions.SEARCH_REPORT:
			serverApi.searchReport(action.data, function (err, response, body) {
				if (body.success) {
					_reportDatas.reports = body.data;
					CentralStore.emit(DASHBOARD_DATA_WATCHER);
				}
			});
			break;
		case Actions.LOAD_REPORTS:
			serverApi.loadReports(action.data, function (err, response, body) {
				if (body.success) {
					var data = body.data;
		...
		case Actions.LOAD_REPORT:
			serverApi.loadReport(action.data, function (err, response, body) {
				if (body.success) {
					_reportDetailData = body.data;
					CentralStore.emit(DASHBOARD_DATA_WATCHER);
				}
			});
			break;
		...
		case Actions.AUTH_SIGNIN:
			...
			serverApi.login(_authDatas.username, _authDatas.password, function (err, response, body) {
				if (body.success) {
					localStorage.setItem('session', JSON.stringify(body.data));
				} else {
		...
		case Actions.REGISTER:
			...
			serverApi.register(user.username, user.password, function (err, response, body) {
				if (!body.success) {
					_registerResult.errorMsg = (body.data ? body.data : 'create user failed, unknow error');
				} else {
					_registerResult.errorMsg = undefined;
				}

				_registerResult.creating = false;
				CentralStore.emit(AUTH_STATE_WATCHER);
			});
			break;
	}
